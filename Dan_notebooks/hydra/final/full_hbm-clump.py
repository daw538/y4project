...
output = pd.read_csv('~/y4project/data/output_clump.csv', usecols=['kic','numax', 'numax_err', 'Phistart'])
#IDs = [np.random.choice(output['kic'], replace=False) for i in range(nstars)]
# Select a random number of stars from the file, recording their IDs
IDs = list(output['kic'].iloc[0:nstars].values)
...
#phi = [(output.loc[(output['kic'] == IDs[i])]).iloc[0]['Phistart'] for i in range(len(IDs))]
...
np.savetxt('no_tau_models_%s' % int(nstars) + '.csv', params, delimiter=',')
hiers.to_csv('no_tau_hiers_%s' % int(nstars) + '.csv', index=False)

if print_fit == True:
	with open('fullsumm_notau_%s' % int(nstars) + '.txt', 'w') as f:
		f.write(summ_notau)
...
no_tau = pd.read_csv('no_tau_models_%s' % int(nstars) + '.csv',
					 names=['kic', 'dnu', 'numax', 'epsilon',
					 'alpha', 'A', 'G', 'phi', 'tau'])
...
df2.to_csv('tau_models_%s' % int(nstars) + '.csv',
			index=False, header=False, na_rep=np.nan)
#np.savetxt('tau_models_%s' % int(nstars) + '.csv', params, delimiter=',')
hiers.to_csv('tau_hiers_%s' % int(nstars) + '.csv')

if print_fit == True:
	with open('fullsumm_tau_%s' % int(nstars) + '.txt', 'w') as f:
		f.write(summ_tau)
